...
!! ,qmin, xls, xlv0, xlf0, den0, denr & ! xls
!! ,cliq,cice,psat &
   real(kind=kind_phys), parameter, private :: &
...
!
         ...
         nc(:,:) = 0.0 ; nr(:,:) = 0.0
!
         !print*,'hli ufs init 1'
         call ufsmpinit(den0,denr,dens,cl,cpv)
         !print*,'hli ufs init 2'
!
...
!-------------------------------------------------------------------------------
   ...
   real(kind=kind_phys) , intent(in ) :: delt, fhswr
...
!-------- initlize CCN -----
     if (kdt==1) then
          ...
          do k=1,nlev
           if(slmsk(i)==0) then
           !nn1(i,k)=10.**(2.41+0.5*log(aerfld(SO4))+0.13*log(1.3*aerfld(OCphilic))+0.05*log(aerfld(Seasalt)))
           ...
           !nn1(i,k)=10.**(2.41+0.5*log(aerfld(SO4))+0.13*log(1.3*aerfld(OCphilic)))
            nn1(i,k)=10.**(2.41+0.5*log(aerfld(i,k,11)*den(k,i)*1.e9)+0.13*log(1.3*aerfld(i,k,15)*den(k,i)*1.e9))*1.e6
           endif
            nn1(i,k)=nn1(i,k)+50.e+6  ! Add a background value for CCN
          enddo
          enddo
      endif
!-------------------------------------------------------------------------------
   ...
   do k = kts,ktop
     do i = its,ite
       if(flgzero) then
         ...
         ncr(k,i,1) = min(max(nn1(i,k),0.2e8),2.e10)
...
!
       ...
       do k = ktop, kts, -1
         if(qrs(k,i,1)>qrmin) then
           ...
           ncr(k,i,3) = max(ncr(k,i,3),nrmin,ncr5mm*den(k,i)*qrs(k,i,1)) ! 5 mm
...
! (qc->qr)
      !if(diac(k)>di15) then
       if(diac(k)>di100) then ! hli exp3a
...
! ncr(k,i,3) = 0.
!----------------------------------------------------------------
! prtoc: conversion from rain to cloud [lh a14]
! (qr->qc)
...
!===============================================================================
   ...
   do k = ktop, kts, -1
       if(cldf(k)>0.) then
         ...
         nraut(k) = nraut(k) * cldf(k)
         nccol(k) = nccol(k) * cldf(k)
         nrcol(k) = nrcol(k) * cldf(k)
         nracw(k) = nracw(k) * cldf(k)
         nsacw(k) = nsacw(k) * cldf(k)
         ngacw(k) = ngacw(k) * cldf(k)
         niacr(k) = niacr(k) * cldf(k)
         nsacr(k) = nsacr(k) * cldf(k)
         ngacr(k) = ngacr(k) * cldf(k)
         naacw(k) = naacw(k) * cldf(k)
         nseml(k) = nseml(k) * cldf(k)
         ngeml(k) = ngeml(k) * cldf(k)
       endif
   enddo
!===============================================================================
!
...
! ncact: nccn -> nc [lh a2] [kk 12]
     if(rh_mul(k)>1.) then
       ncact(k) = max(0.,((ncr(k,i,1) + ncr(k,i,2)) &
                    *min(1.,(rh_mul(k)/satmax)**actk) - ncr(k,i,2)))*rdtcld
...
!
   ...
   pvtr = avtr*g5pbr/24.
   pvtrn = avtr*g2pbr
   print*,' pvtr pvtrn g4pbr ',pvtr,pvtrn, avtr*g4pbr/6.
   eacrr = 1.0
